...
function validateYoutubePlaylistId(ctx, finalCallback) {
    var data = ctx.data || ctx.instance;
    if(_.isEmpty(data.youtubeVars) || _.isEmpty(data.youtubeVars.list)) {
        return finalCallback();
    }

    var err;
    if(!youtubePlaylistRegex.test(data.youtubeVars.list)) {
        err = new Error('Invalid Youtube playlist id');
        err.statusCode = 400;
        err.code = 'INVALID_YOUTUBE_PLAYLIST_ID';
    }
...
function filterDocs(filters) {
    return function(ctx, finalCb) {
      console.log('modelname:', ctx.Model.definition.name);
      ...
      console.log('ctx.instance:', ctx.instance);
        ...
        function addFilter(filter, removeCb) {
          ...
          console.log('typeof ctx.query.where !== "object":', typeof ctx.query.where !== "object");
...
function spamFilter(ctx, finalCb) {
    ...
    async.waterfall([
        ...
        function(user, seriesCb) {
            SpamRegex.find({}, seriesCb);
        }
    ])

    SpamRegex.find({}, function(err, spamRegexes) {
        if(err) {
            return finalCb(filterErr);
        }

        // Get all the different regex
        var regexArr = _.map(spamRegexes, function(spamRegex) { return spamRegex.regex});
