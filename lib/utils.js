...
function generateSlug(sourceKey) {
  return function(ctx, next) {
    ...
    if(typeof data === "object" && typeof newName === "string") {
      ...
      if (typeof slugConfig === "object" && (typeof slugConfig.type === "undefined" ||  slugConfig.type === "object")) {
          
          console.log("slug config is object");
          if (ctx.currentInstance && ctx.currentInstance.slug && !ctx.currentInstance.slug.linked) {
              console.log("current Instance has slug");
              return next();
          }
          if (typeof data.slug !== "object") {
              console.log("slug is not object");
             ...
             data.slug = {
              ...
              console.log("slug.url being changed");
            data.slug.url = newSlug;
          }
      } else {
          console.log("data.slug being changed");
        data.slug = newSlug;
      }
    }
    next();
...
function getCurrentModelInstances(ctx, finalCb) {
    ...
    if(Object.keys(req.currentInstance) > 0) {
