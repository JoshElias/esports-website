...
function destroyChildren(ctx, children, finalCb) {
  ...
  ctx.Model.find(query, function(err, instances) {
    ...
    async.eachSeries(instances, function(instance, instanceCb) {
      async.forEachOfSeries(children, function(child, childModelName, childCb) {
        var childModel = ctx.Model.app.models[childModelName];
        if(!childModel) {
          return childCb();
        }

        var query = {};
        query[child.foreignKey] = instance.id.toString();

        childModel.destroyAll(query, function(err) {
          instanceCb(err);
        });
      }, instanceCb)
    }, function(err) {
