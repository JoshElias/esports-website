...
//            console.log('creating relations');
            async.forEachOf(modelRelations, function (parentRelation, parentRelationName, eachCb) {
				...
				var parentModel = ctx.Model.app.models[parentRelation.model];
...
//                console.log('parentInstance: ', parentInstance);
				...
				var parentRelations = childrenNames || childModel.settings.relations;
				
				
                console.log('parentData: ', parentData);
                async.each(parentData, function (data, parentDataCb) {
					console.log('children names: ', childrenNames);
                    // Get children data from the parent
                    var options = {};
                    for (var parentRelationKey in parentRelations) {
						console.log('child relations: ', parentRelationKey);
                        var childData = data[childRelationName];
                        if (childData) {
                            options[childRelationName] = data[childRelationName];
                        }
                    }
