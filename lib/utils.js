...
function generateSlug(ctx, next) {
    var data;
    var newName;
    if(typeof ctx.data === "object" && typeof ctx.data.name === "string") {
      newName = ctx.data.name;
      data = ctx.data;
    } else if(typeof ctx.instance === "object" && typeof ctx.instance.name === "string") {
      newName = ctx.instance.name;
      data = ctx.instance;
    }
