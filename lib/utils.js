...
//			console.log('relationName:', relationName);
            ...
            function createChildren(relationObj, relationName, childrenCb) {
                //console.log("creating children with relationship name:", relationName);
                ...
                async.each(parentData, function (data, childDataCb) {
                    ...
                    for (var childRelationName in childrenRelations) {
                        var childRelation = childrenRelations[childRelationName];
                        if(!childRelation.isChild) {
                            continue;
                        }

                        var value = data[childRelationName];
                        if (value) {
							console.log('adding value to options: ', value);
                            options[childRelationName] = value;
                        }
                    }
                    ...
                    function createChild(instance, data, options, createCb) {
                        ...
                        instance.create(data, options, function (err, childInstance) {
                            //if (!err) console.log("created child:", relationName, childInstance);
                            createCb(err);
                        });
                    }
