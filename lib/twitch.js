...
function twitchFeedHandler( req, res ) {
	...
	async.waterfall([
		...
		*/
		...
		function(feeds, bullshit, seriesCallback) {
			console.log("6");
...
function getActiveFeed( handles, finalCallback) {
	console.log("Getting active feeds with handle length: "+handles.length);
	if(!Array.isArray(handles)) {
		console.log("Returning active feed early due to no array");
		finalCallback("Unable to get active feed with invalid handles");
		return;
	}
	if(handles.length < 1) {
		console.log("Returning active feed early due to length");
	...
	function(err, streamFeed) {
		...
		console.log("StreamFeed: "+streamFeed)
