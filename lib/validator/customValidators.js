...
function validateUnique(validationState, uniqueCb) {
    ...
    var where = {};
    console.log('validationState.ctx.where:', validationState.ctx.where);
    ...
    Model.find({where: where, fields: {id: true }}, function (err, instances) {
        ...
        console.log('validationState.ctx.currentInstance:', validationState.ctx.currentInstance);
        console.log('instances[0].id:', instances[0].id);
